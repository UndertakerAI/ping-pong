...
int main(void) {
    ...
    while (ScorePlayer1 < WINSCORE && ScorePlayer2 < WINSCORE && end == '\n') {
        if (scanf("%c%c", &command, &end) == 2 && end == '\n' &&
            (command == ' ' || command == 'z' || command == 'Z' || command == 'a' || command == 'A' ||
             command == 'm' || command == 'M' || command == 'k' ||
             command == 'K')) {  // Обработка движения ракеток
...
void print_arena(int ScorePlayer1, int ScorePlayer2, int Rocket1PosY, int Rocket2PosY, int BallPosX,
    ...
    printf("Score first player = %d | Score second player = %d\n", ScorePlayer1, ScorePlayer2);
